...
class HelloWorldParser ( Parser ):
    ...
    class RContext(ParserRuleContext):
        ...
        def ID(self):
            return self.getToken(HelloWorldParser.ID, 0)

        def getRuleIndex(self):
            return HelloWorldParser.RULE_r
    ...
    def r(self):
        ...
        localctx = HelloWorldParser.RContext(self, self._ctx, self.state)
        self.enterRule(localctx, 0, self.RULE_r)
        try:
            self.enterOuterAlt(localctx, 1)
            self.state = 2
            self.match(HelloWorldParser.T__0)
            self.state = 3
            self.match(HelloWorldParser.ID)
